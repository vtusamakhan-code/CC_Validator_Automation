    ...
    for (const [customerName, rowIndices] of customerGroups) {
      ...
      try {
        ...
        if (imageCount === 1) {
          ...
          const imageFile = folder.files[0];
          ...
          try {
            ...
            try {
              ...
              // Generate data URLs for CSV (persistent, self-contained)
              originalImgUrl = await blobToDataURL(originalBlob);
              redactedImgUrl = await blobToDataURL(redactedBlob);
              console.log(`✅ Redaction completed for ${filename}`);
            } catch (redactionError: any) {
              console.warn(`⚠️ Redaction failed for ${filename}:`, redactionError.message);
              // Still generate original URL even if redaction fails
              try {
                originalImgUrl = await blobToDataURL(imageFile);
              } catch (urlError) {
                console.warn(`⚠️ Failed to generate original image URL:`, urlError);
              }
            }
          } catch (error: any) {
            ...
            ccn = 'NOT_FOUND';
            luhnResult = 'Fail';
            // Still try to generate image URLs
            try {
              originalImgUrl = await blobToDataURL(imageFile);
            } catch (urlError) {
              console.warn(`⚠️ Failed to generate original image URL:`, urlError);
          ...
          for (const pair of cardPairs) {
            ...
            if (pair.front && pair.back) {
              ...
              singleFile = folder.files.find(f => 
            ...
            // Call OCR and perform redaction
            let ccn = '';
            let luhnResult = 'Fail';
            const imageUrls: Map<string, { original: string; redacted: string }> = new Map();
            ...
            try {
              ...
              // Perform redaction for each image
              if (pair.front && pair.back && frontFile && backFile) {
                // Paired card - redact separately
                try {
                  ...
                  // Generate URLs for front image
                  const frontOriginalUrl = await blobToDataURL(separateResult.frontOriginal);
                  const frontRedactedUrl = await blobToDataURL(separateResult.frontRedacted);
                  imageUrls.set(frontFile.name, { original: frontOriginalUrl, redacted: frontRedactedUrl });
                  
                  // Generate URLs for back image
                  const backOriginalUrl = await blobToDataURL(separateResult.backOriginal);
                  const backRedactedUrl = await blobToDataURL(separateResult.backRedacted);
                  imageUrls.set(backFile.name, { original: backOriginalUrl, redacted: backRedactedUrl });
                  
                  console.log(`✅ Redaction completed for paired card: ${frontFile.name}, ${backFile.name}`);
                } catch (redactionError: any) {
                  console.warn(`⚠️ Redaction failed for paired card:`, redactionError.message);
                  // Still generate original URLs
                  try {
                    imageUrls.set(frontFile.name, { 
                      original: await blobToDataURL(frontFile), 
                      redacted: '' 
                    });
                    imageUrls.set(backFile.name, { 
                      original: await blobToDataURL(backFile), 
                      redacted: '' 
                    });
                  } catch (urlError) {
                    console.warn(`⚠️ Failed to generate image URLs:`, urlError);
                  }
                }
              } else if (singleFile) {
                // Single image - direct redaction
                try {
                  ...
                  const redactedUrl = await blobToDataURL(redactedBlob);
                  imageUrls.set(singleFile.name, { original: originalUrl, redacted: redactedUrl });
                  
                  console.log(`✅ Redaction completed for ${singleFile.name}`);
                } catch (redactionError: any) {
                  console.warn(`⚠️ Redaction failed for ${singleFile.name}:`, redactionError.message);
                  // Still generate original URL
                  try {
                    imageUrls.set(singleFile.name, { 
                      original: await blobToDataURL(singleFile), 
                      redacted: '' 
                    });
                  } catch (urlError) {
                    console.warn(`⚠️ Failed to generate image URL:`, urlError);
                  }
                }
              }
            } catch (error: any) {
            ...
            for (const idx of rowIndices) {
              ...
              for (const idx of rowIndices) {
                if (!updatedRows[idx]['CCN Actual'] && !updatedIndices.has(idx)) {
                  ...
                  // Try to find matching image URL for this filename
                  const rowFilename = updatedRows[idx].filename;
                  let imageUrlData = { original: '', redacted: '' };
                  for (const [matchFilename, urlData] of imageUrls.entries()) {
                    if (normalizeForComparison(matchFilename) === normalizeForComparison(rowFilename)) {
                      imageUrlData = urlData;
                      break;
                    }
                  }
                  updatedRows[idx] = setLuhnBinResult({
            ...
            if (unprocessedFiles.length > 0) {
              ...
              for (const file of unprocessedFiles) {
                ...
                try {
                  ...
                  // Perform redaction
                  try {
                    const originalBlob = file as Blob;
                    const redactionResponse = await getRedactionMetadata(originalBlob);
                    const redactedBlob = await applyRedactionToImage(originalBlob, redactionResponse.redaction_metadata);
                    
                    originalImgUrl = await blobToDataURL(originalBlob);
                    redactedImgUrl = await blobToDataURL(redactedBlob);
                    console.log(`   ✅ Redaction completed for ungrouped file ${file.name}`);
                  } catch (redactionError: any) {
                    console.warn(`   ⚠️ Redaction failed for ${file.name}:`, redactionError.message);
                    try {
                      originalImgUrl = await blobToDataURL(file);
                    } catch (urlError) {
                      console.warn(`   ⚠️ Failed to generate original image URL:`, urlError);
                    }
                  }
                  ...
                  ccn = 'NOT_FOUND';
                  luhnResult = 'Fail';
                  // Still try to generate original URL
                  try {
                    originalImgUrl = await blobToDataURL(file);
                  } catch (urlError) {
                    console.warn(`   ⚠️ Failed to generate original image URL:`, urlError);
